...
module.exports = (artistsRouter, db) => {
  ...
  artistsRouter.get("/:artistId", async (ctx, next) => {
    ...
    if (!artist) return errorMiddleware.send404(ctx, next)
  ...
  artistsRouter.get("/by-name/:str", async (ctx, next) => {
    ...
    if (R.isEmpty(artists)) return errorMiddleware.send404(ctx, next)
  ...
  artistsRouter.post("/", async (ctx, next) => {
    ...
    if (!name) return errorMiddleware.send400(ctx, next)
